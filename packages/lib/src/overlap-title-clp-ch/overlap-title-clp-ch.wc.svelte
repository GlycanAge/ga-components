<svelte:options customElement={{ tag: 'ga-overlap-title-clp-ch', shadow: 'open' }}/>
  ...
  <div class="main" style="background-color: {bgColor};">
    <div class="container">
      {#if message === 'no significant overlap'}
        <p>当前结果与此病症之间 <b style="color: {color};">没有显著的糖化指标重叠</b>。</p>
      {:else}
        <p>当前结果与此病症之间存在 <b style="color: {color};">轻微的糖化指标重叠</b>。</p>
      {/if}
      {#if message === 'no significant overlap'}
