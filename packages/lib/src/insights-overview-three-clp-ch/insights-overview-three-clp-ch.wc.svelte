<svelte:options customElement={{ tag: 'ga-insights-overview-three-clp-ch', shadow: 'open' }}/>
...
{#if show}
  {#if type === 'long'}
    <div class="main">
      <div class="row">
        <div style="flex: 1;"><b>心血管健康</b></div>
        {#if cardioFinalGreen}
      ...
      <div class="row">
        <div style="flex: 1;"><b>呼吸系统健康</b></div>
        {#if respGreen}
          <div class="cause-chip good">
            <div class="cause-chip-dot"></div>
            最佳
      ...
      <div class="causes mb-6">
        {#each badAreas as area}
          <div class="cause-chip {area.color === 'red' ? 'bad' : 'slightly-bad'}">
            <div class="cause-chip-dot"></div>
            {getTranslation('chinese', area.label)}
          </div>
        {/each}
      </div>
      <p class="x-small" style="max-width: 90%;"><b>请注意</b>: 虽然某些模式可能与特定病症中观察到的模式相似，但这并不能确诊疾病或其任何亚临床形式。</p>
    {/if}
  {/if}
{/if}
