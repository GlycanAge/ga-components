<svelte:options customElement={{ tag: 'ga-arrow-graph-three', shadow: 'open' }}/>

<script lang="ts">
  import {onMount} from 'svelte';
  import {Service} from '../shared/utils/service';
  import {getTranslation} from '../shared/functions/helpers';
  import Arrow3 from '../shared/components/Arrow3.svelte';
  ...
  function getMessage() {
    if (direction === 'right') {
      if (percentile <= 66) {
        return getTranslation(lang, 'OPTIMAL');
      } else {
        return getTranslation(lang, 'SUBOPTIMAL');
      }
    } else {
      if (percentile >= 34) {
        return getTranslation(lang, 'OPTIMAL');
