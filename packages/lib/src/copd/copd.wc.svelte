...
<script lang="ts">
    ...
    let min1 = 0; // G1xmin
    let max1 = 0; // G1xmax
    let mid1 = 0; // G1xaverage
    let res1 = 0; // G1yourscore
    let perc1 = 0; // G1percentile

    let min2 = 0; // Bxmin
    let max2 = 0; // Bxmax
    let mid2 = 0;  // Bxaverage
    let res2 = 0; // Byourscore
    let perc2 = 0; // Bpercentile
    ...
    onMount(async () => {
        ...
        min1 = Number(reportData.G1xmin);
        max1 =  Number(reportData.G1xmax);
        mid1 =  Number(reportData.G1xaverage);
        res1 =  Number(reportData.G1yourscore);
        perc1 =  Number(reportData.G1percentile);
        min2 =  Number(reportData.Bxmin);
        max2 =  Number(reportData.Bxmax);
        mid2 =  Number(reportData.Bxaverage);
        res2 =  Number(reportData.Byourscore);
        perc2 =  Number(reportData.Bpercentile);

        if (res1 < mid1 && res2 > mid2) {
            overlap = true;
            showSummary = true;
            return;
        }
            ...
            <div class="summaryMain">
                ...
                <div class="summaryBody" style="opacity: 0.35;">
                    ...
                    <div style="font-size: 0.7rem; padding-bottom: 0.4rem;"><b>Shortness of Breath:</b> Due to reduced airflow and decreased oxygen <br>
                        exchange in the lungs.</div>
                    <div style="font-size: 0.7rem; padding-bottom: 0.4rem;"><b>Wheezing:</b> Indicates narrowing or obstruction in the bronchial tubes, <br>
                        typical in emphysema.</div>
                    <div style="font-size: 1.2rem; padding-top: 1.2rem;padding-bottom: 1rem;">Possible follow-up tests:</div>
                    <div style="font-size: 0.7rem; padding-bottom: 0.4rem;"><b>Spirometry:</b> Measures the amount and speed of air a person can exhale, <br>
                        crucial for diagnosing COPD</div>
                    <div style="font-size: 0.7rem; padding-bottom: 0.4rem;"><b>Chest X-Ray:</b> Can show emphysema, one of the main diseases under the <br>
                        COPD umbrella, and rule out other lung problems.</div>
                    <div style="font-size: 0.7rem; padding-bottom: 0.4rem;"><b>Arterial Blood Gas Analysis:</b> Assesses the oxygen and carbon dioxide levels <br>
                        in the blood, indicating how well lungs are functioning.</div>
                </div>
            </div>
        {:else}
...
{:else}
    <div class="main">
        ...
        <div class="lastRow">
            <div class="dot1"></div>
            <div style="font-size: 0.8rem;">COPD</div>
            <div class="dot2"></div>
