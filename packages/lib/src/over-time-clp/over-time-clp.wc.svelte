<svelte:options customElement={{ tag: 'ga-over-time-clp', shadow: 'none' }}/>
...
<script lang="ts">
  ...
  function drawGraph() {
    ...
    var glycanData = overTimeData.data.map((item, index) => [index, item.si]);
    ...
    var option = {
      ...
      yAxis: {
        min: Math.min(...overTimeData.data.map(item => Math.min(item.x, item.si))) - 10,
        max: Math.max(...overTimeData.data.map(item => Math.max(item.x, item.si))) + 10,
        name: '',
        axisLine: {
          show: false,
          symbol: ['none', 'arrow'],
      ...
      if (j !== resultIndex) {
        option.graphic.push({
          ...
          shape: {
            ...
            y2: chart.convertToPixel('yAxis', Math.max(...overTimeData.data.map(item => Math.max(item.x, item.si))) + 10)
          },
          style: {
            stroke: '#09341F33',
            lineDash: [6, 15],
    ...
    for (let k = 0; k < overTimeData.data.length; k++) {
      if (overTimeData.data[k].x < overTimeData.data[k].si || overTimeData.data[k].x === overTimeData.data[k].si) {
        const newSvg = document.createElementNS("http://www.w3.org/2000/svg", "svg");
        const scoreHolder = document.createElement('div');
        ...
        path1.setAttribute("fill", getColor(overTimeData.data[k].x, overTimeData.data[k].si));
        newSvg.appendChild(path1);
        newSvg.style.position = 'absolute';
        ...
        scoreHolder.innerText = `${overTimeData.data[k].si}`;
        scoreHolder.style.position = 'absolute';
        ...
        scoreHolder.innerText = `${overTimeData.data[k].si}`;
        scoreHolder.style.position = 'absolute';
        scoreHolder.style.left = `${chartRect.left + chart.convertToPixel('xAxis', k) - (overTimeData.data[k].si < 10 ? 4 : 7)}px`;
  ...
  onMount(async () => {
    ...
    overTimeData.data.map((it) => {
      if (it.y > it.x) {
        it.si = Math.round(50 + (it.y - it.x) / (80 - it.x) * 50);
      } else if (it.y < it.x) {
        it.si = Math.round(50 - (it.x - it.y) / (it.x - 20) * 50);
      } else {
        it.si = 50;
      }
    });
